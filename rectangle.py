...
class Toolbar(QtGui.QToolBar):
    ...
    def __init__(self, view):
        ...
        self.view = view
        self.toolbar_rectangle()
        self.toggleViewAction().setText("Prostokąt")
        self.setVisible(False)
        icon = QtGui.QIcon("img/tools/rectangle.png")
        self.toggleViewAction().setIcon(icon)
        self.view.tool = 1


    def toolbar_rectangle(self):
...
class Drawing(QtGui.QGraphicsView):
    ...
    def drawing(self, start, end, color, width, style, color_brush, brush):

        self.rectangle = QtGui.QGraphicsRectItem(QtCore.QRectF(start, end))
        self.rectangle.setPen(QtGui.QPen(QtGui.QColor(str(color)), width, style))
        self.rectangle.setBrush(QtGui.QBrush(QtGui.QColor(str(color_brush)), brush))
        command = Draw(self.scene, self.rectangle)
        self.undoStack.push(command)


class Draw(QtGui.QUndoCommand):

    def __init__(self, scene, rectangle):
