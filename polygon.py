...
class Toolbar(QtGui.QToolBar):
    ...
    def __init__(self, view):
        ...
        self.view = view
        self.toolbar_polygon()
        self.toggleViewAction().setText("Wielokąt")
        self.setVisible(False)
        icon = QtGui.QIcon("img/tools/polygon.png")
        self.toggleViewAction().setIcon(icon)
        self.view.tool = 1


    def toolbar_polygon(self):
...
class Drawing(QtGui.QGraphicsView):
    ...
    def drawing(self, A1, A4, color, width, style, color_brush, brush):
        ...
        c = QtCore.QPointF(A4.x()+all,A1.y()+all)
        d = A4
        e = QtCore.QPointF(A1.x(),A4.y())
        f = QtCore.QPointF(A1.x()-all,A1.y()+all)

        list = [a,b,c,d,e,f]

        self.polygon = QtGui.QGraphicsPolygonItem(QtGui.QPolygonF(list))
        self.polygon.setFlag(QtGui.QGraphicsItem.ItemIsMovable)
        self.polygon.setPen(QtGui.QPen(QtGui.QColor(str(color)), width, style, ending))
        self.polygon.setBrush(QtGui.QBrush(QtGui.QColor(str(color_brush)), brush))
        command = Draw(self.scene, self.polygon)
